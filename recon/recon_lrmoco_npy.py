...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_ref', type=int, default=-1,
                        help='reference frame for reconstruction')
    ...
    parser.add_argument('--vent_flag', type=int, default=0,
                        help='output jacobian determinant & specific ventilation')    
...
# Delete some unused arrays to save memory
    ...
    for i in range(outer_iter):
        ...
        for j in range(iner_iter):
            ...
            logging.info('outer iter:{}, inner iter:{},res:{}'.format(i,j,res_norm))
            if res_norm<5e-8:
                break
        ...
        if mr_cflag==1:
            np.save(os.path.join(fname, 'mocolor.npy'), qt)
        else:
            np.save(os.path.join(fname, 'lor.npy'), qt)
        #np.save(os.path.join(fname, 'mocolor_residual_{}.npy'.format(lambda_lr)), np.asarray(res_list))
        
    # jacobian determinant & specific ventilation
    # qt = np.load(os.path.join(fname, 'mocolor.npy'))
    if vent_flag==1:
        ...
        svs = np.asarray(svs)
        if mr_cflag==1:
            np.save(os.path.join(fname, 'jac_mocolor.npy'), jacs)
            np.save(os.path.join(fname, 'sv_mocolor.npy'), svs)
        else:
